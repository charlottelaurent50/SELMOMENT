...
class AdminController extends AbstractController
{
    ...
    public function sendMonthlyAnnouncements(ManagerRegistry $doctrine, string $currentDate)
{
    $currentDate = new \DateTime($currentDate);
    $currentYearMonth = $currentDate->format('Y-m-d');

    $annonces = $doctrine->getRepository(Annonce::class)->findCurrentMonthAnnouncements($currentYearMonth);
    ...
    foreach ($comptes as $compte) {
        if ($compte->getEmail() != null){
            $this->sendEmail($compte, $annonces, $currentDate);
        }
    }
    return $this->render('admin/index.html.twig', [
        'controller_name' => 'AdminController',
    ...
    private function sendEmail(Compte $compte, array $annonces, \DateTime $currentDate)
    {
        $email = (new TemplatedEmail())
            ->from(new Address('contact@selmoment.fr', 'SEL Moment !'))
            ->to($compte->getEmail())
            ->subject('Découvrez les dernières annonce du SEL')
            ->htmlTemplate('email/monthly_announcements.html.twig')
            ->context(['user' => $compte, 'annonces' => $annonces, 'currentDate' => $currentDate]);;
